...
FROM (
  ...
  FROM (
    ...
    FROM `fh-bigquery.wikidata.wikidata_latest_20190822`, UNNEST(instance_of)
    GROUP BY 1 
  ) a
...
LOOP
  BEGIN
    ...
    SET row_count = (SELECT COUNT(*) FROM `wikidata.instances_20190822`) 
    ;


    INSERT INTO `wikidata.instances_20190822`
    ...
    FROM `wikidata.instances_20190822` a
    JOIN `wikidata.instances_20190822` b
    ON a.instance_of_id = b.id
    WHERE CONCAT(a.id,b.instance_of_id) NOT IN (SELECT CONCAT(id,instance_of_id) FROM `wikidata.instances_20190822`)
    GROUP BY 1,2,3,4,5,6,7
    ;

    SET row_diff = (SELECT COUNT(*) FROM `wikidata.instances_20190822`) - row_count;
    IF row_diff = 0 THEN
        LEAVE;
    END IF;
  END;
