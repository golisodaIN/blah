SELECT views, title
   , ARRAY(SELECT DISTINCT AS STRUCT en_label, numeric_id FROM `fh-bigquery.wikidata.subclasses_20190822` a JOIN UNNEST(occupation) b USING(numeric_id)) occupation
   , ARRAY(SELECT DISTINCT AS STRUCT en_label, numeric_id FROM `fh-bigquery.wikidata.subclasses_20190822` a JOIN UNNEST(instance_of) b USING(numeric_id)) instance_of
FROM (
