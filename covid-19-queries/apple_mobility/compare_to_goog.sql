WITH data AS (
  ...
  SELECT country_region, google_type, b.transportation_type apple_type, google, b.value apple
  FROM unpivotted a 
...
SELECT country_region, google_type, apple_type, ROUND(100*CORR(google, apple),2) corr, COUNT(*) days, ROUND(AVG(100+google),1) avg_goog, ROUND(AVG(apple),1) avg_appl
FROM joined
WHERE google_type = 'transit' AND apple_type='transit'
GROUP BY 1,2,3
-- HAVING ABS(corr) > 80
ORDER BY google_type DESC, corr DESC
