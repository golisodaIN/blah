CREATE TEMP FUNCTION parse_n(x ANY TYPE) AS (
 CAST(REGEXP_EXTRACT(x, '^.(.*)') AS INT64)
);
...
CREATE TEMP FUNCTION route(x ANY TYPE) AS ((
  SELECT ARRAY_AGG(a)
  FROM (
    SELECT *, -n-1+ROW_NUMBER() OVER(ORDER BY n, n1, n2) rn
    FROM (
      SELECT *, SUM(move.x) OVER(ORDER BY n) x , SUM(move.y) OVER(ORDER BY n) y 
      FROM (
        SELECT n, d, parse(d) move
        FROM UNNEST(SPLIT(x)) d
        WITH OFFSET n
      )
    )
